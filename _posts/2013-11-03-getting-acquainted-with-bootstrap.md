...
featured:
  src: /assets/img/Bootply_-_The_Bootstrap_Playground.png
  alt: Menu navigation styled with Bootstrap
  caption: A navigation box written in HTML and styled using Bootstrap
excerpt:
 ...
 * [Pintrest posts tagged #Bootstrap](http://www.pinterest.com/search/?q=Bootstrap)
 * [The official bootstrap “expo”](http://expo.getbootstrap.com/)
...
For the second option, go to [The Bootstrap Playground](http://bootply.com). It's a site that allows you to test out Bootstrap code in your browser and see view the results in almost real-time. Start a new "playground" and try loading some of the templates and community-contributed code snippets. See if you can figure out what the code snippets do. Try loading the html from the template you downloaded in the previous step. Can you get it to load properly? You might need to include some additional CSS or javascript to get things to look and function correctly. Bootply is still a little rough around the edges, so it's not a big deal if things don't work out or you are totally confused by it.

The third option is to use a visual layout editor. I recommend checking out [Divshot](https://divshot.com/blog/product-updates/divshot-1-0-visual-front-end-development-for-bootstrap/). They have a free plan (still requires registering) that should be plenty for this mini-course. Try it out and definitely check out their very thorough tutorial. [Easel.io](https://www.easel.io/demo) is also very slick and works well, and like divshot, they have a free plan, but theirs only lets you save up to 3 pages.

You might also check out [Pingendo](http://www.pingendo.com/), which is a free visual editor that runs on Windows or OS X, but is still very rough around the edges and isn't very drag-and-drop friendly. There is also [LayoutIt](http://www.layoutit.com/) which seems like it works decently, but I haven't spent enough time playing with it to know how well it works compared to divshot or easel.io.
